...
flag_src_diff_stfs=0; % 1 for different stfs; 0 - same stf
...
elseif flag_assign_value == 2 
    ...
    transducer_number3 = 1; %  transducer_number in one col, (num of row) 

    transducer_length=0.01;  % transducer length in one row, 
    transducer_length2=0.01;  % transducer length in one col,  
    transducer_length3=0;  % transducer length in one col,  
...
%     
    name_of_source_folder = strcat(sysHOME, '/csp2d_base/DATA/sources3d/mh/')
    name_of_source_file = strcat(sysHOME, '/csp2d_base/DATA/sources3d/mh/0000')
end
